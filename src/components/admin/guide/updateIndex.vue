...
const get_GuideList = () => {
  getGuideList(`id=${information.id}`).then((res) => {
    grade.value = res.data.years;
    academicDegree.value = res.data.degree;
    type.value = res.data.type;
    guideName.value = res.data.title;
    guideContent.value = res.data.content;
    filesUrl.value = res.data.filesUrl; // 上传指南路径
    filesName.value = res.data.filesName; // 上传指南名称
    uploadcontent.value =
      filesUrl.value != ""
        ? [{ name: filesName.value, url: filesUrl.value }]
        : [];
    getSemester(`guideId_EQ=${res.data.id}`).then((res) => {
      // console.log(res);
      semester.value = String(res.data.total);

      for (let index = 0; index < res.data.total; index++) {
        dataids.value.push(res.data.rows[index].id);
        semesterName.value.push(res.data.rows[index].term);
        semesterDate.value.push([
          res.data.rows[index].startTime,
          res.data.rows[index].endTime,
        ]);
      }
    });
  });
...
const oper = (text: boolean) => {
  ...
  if (!text) {
    updateGuide({
      ...
      const addSemester = ref<Array<any>>([]);
      // 修改学期
      for (let index = 0; index < Number(semester.value); index++) {
        const aaa = {
          id: dataids.value[index],
          guideId: information.id,
          term: semesterName.value[index],
          startTime: moment(semesterDate.value[index][0]).format("YYYY-MM-DD"),
          endTime: moment(semesterDate.value[index][1]).format("YYYY-MM-DD"),
          minScore: "0",
          maxScore: "10000",
        };
        if (dataids.value.length - 1 >= index) {
          updateSemester(aaa).then((res) => {});
        } else {
          addSemester.value.push(aaa);
        }
      }
      // 新增学期
      setSemester(addSemester.value).then((res) => {});
      // end
      // 删除学期
      if (Number(semester.value) < dataids.value.length) {
        let idArr = dataids.value.splice(
          Number(semester.value),
          dataids.value.length
        );
        deleteSemester({ ids: idArr.join(",") }).then((res) => {});
      }
      // end
      console.log("123");
