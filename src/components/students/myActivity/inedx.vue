      ...
      >
        <div class="w-full flex">
          ...
          <myElselect
            ...
            :placeHolder="'全部'"
            :fontSize="18"
            :arr="myActiveStateArr"
            @choice="myActiveStateChoice"
          ></myElselect>
          <myElselect
            :text="'活动类别'"
            :placeHolder="'全部'"
...
const myActiveStateArr = useTestStore().myActiveState;
const myActiveStateChoice = (text: string) => {
...
const details = (id: string) => {
  emit("my_state", "1");
  // emit("id", id);
  instance?.proxy?.$Bus.emit("get", id);
};
// end
